...
from fmri_connectivity import *

### Choose the base directory where is the bids database

source_dir = "/Volumes/LaCie/nifti3/nifti3"
group = "Patients"
output_dir = "/Volumes/LaCie/derivatives"
### Subject select
...
if clean_hidden_files:	
	for sub in subject_list:
		ses001_hidden_files = [f"._{sub}_ses-001_task-rest_bold.json",f"._{sub}_ses-001_T1w.nii.gz"]
		...
		for hidd in ses001_hidden_files:
			print(hidd)
...
if run_preprocessing: 
	...
	for sub in subject_list_CLI:
		...
		command = f"docker run --rm -e DOCKER_VERSION_8395080871=24.0.7 -it -v /opt/freesurfer/license.txt:/opt/freesurfer/license.txt:ro -v {source_dir}/{group}:/data:ro -v {output_dir}/{group}:/out nipreps/fmriprep:23.2.0 /data /out participant --participant-label {sub} --skip-bids-validation --fs-no-reconall"
		print(command)
